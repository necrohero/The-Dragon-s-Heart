...
function s.filter(c,e,tp) --filters for a bujin on the field
...
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc) --targets a dragon xyz on the  field
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and chkc:IsAbleToRemove() and chkc:IsRace(RACE_DRAGON) and chkc:IsType(TYPE_XYZ) end
